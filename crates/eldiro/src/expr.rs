...
impl Number {
    fn new(s: &str) -> Result<(&str, Self), String> {
        let (s, number) = utils::extract_digits(s)?; // ? unwraps Ok and returns Err if its an Err
        ...
        match self {
            ...
            Self::Operation { lhs, rhs, op } => {
                ...
                let (lhs, rhs) = match (lhs, rhs) {
                    ...
                    _ => {
                        return Err(
                            "Cannot evaluate operation whose lhs and rhs are not numbers"
                                .to_string(),
                        )
                    }
                };
...
#[cfg(test)]
mod tests {
    use super::*; // Needed to use Number, etc here
    use crate::stmt::Stmt;
